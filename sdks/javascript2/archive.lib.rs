...
static mut connections:HashMap<u8, Connection> = HashMap::new();

fn newDbConn(arg: String) -> JsResult<JsString> {
    let id = Uuid::new_v4();

    let c = Rc::new(RefCell::new(new_connection(arg).expect("Couldn't open conn.")));
...
fn newMemoryConn(arg: Type) -> RetType {
    unimplemented!()
}

fn transact(arg: Type) -> RetType {
    unimplemented!()
}

fn query(arg: Type) -> RetType {
    unimplemented!()
}
...
register_module!(m, {
    let connection_class = (JsConnection::class(m.scope))?;
    let connection_constructor = (connection_class.constructor(m.scope))?;
    (m.exports.set("Connection", connection_constructor))?;
