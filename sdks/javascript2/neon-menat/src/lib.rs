...
trait MyFunctionContext<'a>: Context<'a>{
    fn argument<V: Value>(&mut self, i: i32) -> JsResult<'a, V>;
}

// impl<'a> MyFunctionContext for FunctionContext<'a> {
//     fn argument<V>(&mut self, _: i32) -> Result<neon::handle::Handle<'a, V>, Throw> where V: neon::prelude::Value { todo!() }
// }
...
impl Database {
    ...
    //    to the connection.
    // fn new<'a>(cx: &mut FunctionContext) -> Result<&'a Self, Box<Error>>
    fn new<'a, C: MyFunctionContext<'a>>(cx: &mut C) -> Result<Self, Box<Error>>
        where
            C: Context<'a>,
    {
        let js_conn_str = cx.argument::<JsString>(0)?;
        // println!("{}", js_conn_str.value(&mut cx).to_string());
        // let conn_str = js_conn_str.value(&mut cx).to_string()
        // conn_str.value(&mut cx).to_string();

        // let db = new_connection(conn_str.value(&mut cx).to_string()).expect("Couldn't open conn.");
        // let mentat_db = conn::Conn::connect(&mut db.borrow_mut()).expect("Couldn't open DB.");
        ...
        let mut conn = new_connection("test.db").expect("Couldn't open conn.");
